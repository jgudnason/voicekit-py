...
# (name, fs, n_samples, f0 at start, f0 at end)
INPUTS = [
    ("vowel_f0100_16k", 16000, 9600, 100.0, 100.0),  # 9600 % 8 == 0: no-pad path
    ("vowel_glide_16k", 16000, 8837, 100.0, 150.0),  # 8837 % 8 == 5: pad/trim path
    ("vowel_f0120_8k", 8000, 4801, 120.0, 120.0),  # 4801 % 8 == 1: pad + fs<9000 branch
]


def rosenberg_pulse(t0: int) -> np.ndarray:
...
def glottal_flow(fs: int, n: int, f0_start: float, f0_end: float) -> np.ndarray:
...
def synth_vowel(fs: int, n: int, f0_start: float, f0_end: float) -> np.ndarray:
    u = glottal_flow(fs, n, f0_start, f0_end)
    ...
    return np.asarray(0.95 * speech / np.max(np.abs(speech)))


def main() -> None:
    for name, fs, n, f0_start, f0_end in INPUTS:
        samples = synth_vowel(fs, n, f0_start, f0_end)
        path = FIXTURES_DIR / f"{name}.wav"
        write_wav(Signal(samples=samples, fs=fs), path)
